...
ctx:fillRect(300,25, flag_w,flag_h/13)
--畫白第二條線
ctx.fillStyle='rgb(999, 999, 999)'
...
function star(x, y, r, angle, solid, color)
    ...
    for i = 0, 5 do
        xout = (x + r*math.sin((360/6)*deg*i+angle*deg))
        yout = (y + r*math.cos((360/6)*deg*i+angle*deg))
        -- 外點增量 + 1
        xout2 = x + r*math.sin((360/6)*deg*(i+1)+angle*deg)
        yout2 = y + r*math.cos((360/6)*deg*(i+1)+angle*deg)
        xin = x + rin*math.sin((360/6)*deg*i+36*deg+angle*deg)
        yin = y + rin*math.cos((360/6)*deg*i+36*deg+angle*deg)
        -- 查驗外點與內點座標
...
for i = 0, 6 do
    for j = 0, 6 do
        star(15+45*i, -23+35*j,10, 0, true, "#fff")
    end
end